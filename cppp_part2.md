...
### 12.2.1 new 和数组
让 new 分配对象数组，需要在类型名之后跟一堆方括号，其中指明要分配的对象的数目
```cpp
...
```
- 分配一个数组会得到一个元素类型的指针
    - new 返回一个元素类型的指针——动态数组不是数组类型
    - 不能对动态数组调用 begin 或 end
- 初始化动态分配对象的数组
    - 默认情况下，new 分配的对象，不管是单个分配的还是在数组中的，都是默认初始化的，
    - 可以对数组中的元素进行值初始化，在大小之后跟一堆空括号
        - 新标准中可以提供一个元素初始化器的花括号列表
- 动态分配一个空数组是合法的
    - 当使用 new 分配一个大小为 0 的数组是，new 返回一个合法的飞空指针，辞职真类似尾后指针，可以像使用尾后迭代器一样使用这个指针
- 释放动态数组
    - 在指针前加上一个空方括号对
    - 在 delete 一个指向数组的指针时忽略了方括号，其行为是未定义的
- 智能指针和动态数组
    - 标准库提供了一个可以管理 new 分配的数组的 unique_ptr 版本，为了用一个 unique_ptr 管理动态数组，需要在对象类型后面跟一堆空方括号
    - 指向数组的 unique_ptr 不支持成员访问运算符(点和箭头运算符)
        1. `unique_ptr<T[]> u` u 可以指向一个动态分配的数组，数组元素类型为 T
        2. `unique_ptr<T[]> u(p)` u 指向内置指针 p 所指向的动态分配的数组，p 必须能转换为类型 T* 
        3. `u[i]` 返回 u 拥有的数组中位置 i 处的对象
    - shared_ptr 不直接支持管理动态数组，如果需要，必须提供自己定义的删除器
        ```cpp
